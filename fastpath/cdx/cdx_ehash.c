...
/* Linux ALIGN(x, a) rounds x up to next multiple of a.
 * FreeBSD defines ALIGN(x) (1-arg) in sys/param.h — undefine it. */
#undef ALIGN
...
#ifdef __FreeBSD__
/*
 * SYS_VirtToPhys is an SDK function that doesn't exist on FreeBSD.
 * Map it to NCSW's XX_VirtToPhys which handles MURAM/DDR translation.
 */
#define SYS_VirtToPhys(addr) XX_VirtToPhys((void *)(uintptr_t)(addr))
...
/*
 * FmMurambaseAddr — base virtual address of MURAM.
 * On Linux SDK this is a global in fm_ehash.c.  On FreeBSD it's set
 * by cdx_dpa_bridge.c from t_FmPcd->p_FmMuramMemBaseAddr.
 */
...
#ifndef __FreeBSD__
/*
 * On Linux SDK, this wraps SYS_VirtToPhys for NCSW compatibility.
 * On FreeBSD, NCSW already provides XX_VirtToPhys in xx_arm64.c,
 * and SYS_VirtToPhys is mapped to it via #define above.
 */
...
int insert_entry_in_classif_table(PCtEntry entry)
{
	...
	if (!info)
		return FAILURE;
	info->entry = entry;
	// Can be NULL if the route goes through an interface CDX doesn't
	// know about (e.g. bridge0).
	underlying_input_itf = entry->pRtEntry->underlying_input_itf;
	if (underlying_input_itf == NULL) {
		DPA_INFO("%s::underlying_input_itf is NULL for port %d "
...
{
	struct L2Flow_entry *entry __unused;
	int ii;
...
#endif
skip_stats: __attribute__((unused));
	/* write word */
	param->word = cpu_to_be32(word);
...
{
	struct en_ehash_preempt_op *param;
	uint8_t mtu_off_val, opmask_val;

	if (!info->enqueue_params || !info->preempt_params)
		return FAILURE;
	param = (struct en_ehash_preempt_op *)info->preempt_params;
	mtu_off_val = (uint8_t)(info->enqueue_params - info->preempt_params);
	opmask_val = 0;
	if (!info->l2_info.is_wlan_iface)
		opmask_val |= PREEMPT_TX_VALIDATE;
	if (!(info->flags & EHASH_IPV6_FLOW))
		opmask_val |= PREEMPT_DFBIT_HONOR;

	param->mtu_offset = mtu_off_val;
	param->OpMask |= opmask_val;
...
#if defined(VOIP_PRIORITY_SLOW_PATH_FRAME_QUEUES) && defined(__LP64__)
#error "RTP relay uses 32-bit pointer fields (en_ehash_rtprelay_param) — not ported to 64-bit"
#endif
...
#ifndef __FreeBSD__
/* proc_fs-based debug/stats interface — Linux only */
...
#ifndef __FreeBSD__
/* Fragment buffer pool — uses Linux BMan pool APIs, not available on FreeBSD */
