...
int dpa_add_lagg_if(char *name, struct _itf *itf, struct _itf *phys_itf,
		uint8_t *mac_addr, POnifDesc *member_onifs, int num_members,
		uint32_t lagg_proto);
int dpa_get_lagg_member_ports(uint32_t itf_index, uint32_t *port_ids,
		int max, int *count);
int dpa_get_broadcast_lagg_members(uint32_t itf_index,
